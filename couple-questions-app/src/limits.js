...
export function isBlockOpen(catId, profile, connections = []) {
  if (hasAccess(profile, connections)) return true;
  return freeBlockIds(profile.topics).includes(catId);
}

export function canAddConnection(profile, connections) {
  return hasAccess(profile, connections) || connections.length < FREE_CONNECTIONS;
}

/** Сколько вопросов закрыто — для текста на экране покупки. */
export function lockedQuestionCount(profile, connections = []) {
  if (hasAccess(profile, connections)) return 0;
