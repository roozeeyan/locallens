...
def groq_describe(name: str, category: str, city_id: str, travel_around: bool = False) -> str:
    ...
    BANNED = (
        "НЕ используй слова и фразы: 'уютный', 'уютное', 'уютная', 'уютный уголок', "
        "'идеальное место', 'настоящая находка', 'жемчужина', 'в сердце', 'в центре', "
        "'расположен', 'находится', 'предлагает', 'можно насладиться'.\n"
        "Не пиши географическое положение ('в центре города', 'на улице X'). "
        "Вместо этого — конкретная деталь: блюдо, интерьер, концепция, атмосфера, фишка."
    )

    if travel_around:
        country = city.split(",")[-1].strip()
        prompt = (
            f'Напиши описание места "{name}" — это {cat_hint}, расположен за городом в {country}.\n'
            "1–2 предложения. Опиши природу, атмосферу, маршрут или что там делают.\n"
            f"{BANNED}\n"
...
def main():
    ...
    for pid, city_id, category, name, has_hardcoded, travel_around in places:
        ...
        if args.fix_language:
            ...
            # Process entries with cliche openers OR overused filler words
            low = existing.lower()
            bad_openers = ("в сердце", "в центре", "в самом сердце", "расположен ", "находится в ")
            bad_anywhere = (
                "уютный уголок", "уютное место", "идеальное место",
                "настоящая находка", "жемчужина города",
                " расположен ", " расположена ", " расположено ",
                " находится ", " уютный ",
            )
            is_template = (
                any(low.startswith(p) for p in bad_openers)
                or any(p in low for p in bad_anywhere)
            )
            if not is_template:
                skipped += 1
                continue
